...
public class StudyChecker
{
    ...
    {
        /*
        sum(fetData$BindCount) # 28482
        sum((fetData %>% filter(AminoAcid=='A'))$BindCount) # 876
        sum((fetData %>% filter(Study=='Admon - cellular MHC/mass spectrometry'))$BindCount) # 21664
        sum((fetData %>% filter(AminoAcid=='A'&Study=='Admon - cellular MHC/mass spectrometry'))$BindCount) # 829
        sum((fetData %>% filter(AminoAcid=='A'&Study!='Admon - cellular MHC/mass spectrometry'))$BindCount) # 47
        sum((fetData %>% filter(AminoAcid!='A'&Study=='Admon - cellular MHC/mass spectrometry'))$BindCount) # 20835
        sum((fetData %>% filter(AminoAcid!='A'&Study!='Admon - cellular MHC/mass spectrometry'))$BindCount) # 6771
        expected = (876 * 21664) / 28482
        print(expected)

        # study = G1, amino-acid = G2
        fishMatrix = rbind(c(829,47),c(20835,6771))
        */
    ...
    public static void main(@NotNull final String[] args) throws ParseException
    {
        ...
        options.addOption(STUDY_DATA_FILE, true, "Output filename");
