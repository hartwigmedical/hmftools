...
public enum JitterMatch
{
            ...
            final JitterMatch jitterType, boolean jitterAtStart, int jitterCount)
    {
        ...
        for(; readIndex <= flankReadIndexEnd; ++readIndex, ++readContextIndex)
        {
            ...
            if(readQuals[readIndex] <= 1 && readContextIndex >= permittedLowQualRangeLower && readContextIndex <= permittedLowQualRangeUpper)
                return false;  // never count jitter for a core that contains 0 qual bases in essential region. this makes it comparable to core matching logic
