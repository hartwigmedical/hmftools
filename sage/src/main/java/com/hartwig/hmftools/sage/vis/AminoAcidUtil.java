...
public final class AminoAcidUtil
{
    ...
    public static List<GeneRegionViewModel> getGeneRegions(final TranscriptData transcriptExons)
    {
        ...
        for(int i = idxStart; i >= 0 && i < exons.size(); i += inc)
        {
            ...
            {
                if(posStrand)
                    geneRegions.add(new IntronicRegionViewModel(new BaseRegion(prevExon.End + 1, exon.Start - 1), 0));
                else
                    geneRegions.add(new IntronicRegionViewModel(new BaseRegion(exon.End + 1, prevExon.Start - 1), 0));
            }
            ...
            {
                geneRegions.add(new NonCodingExonicRegionViewModel(exonRegion, 0));
                continue;
            }
            ...
            if(posStrand)
            {
                if(exon.Start < codingStart)
                    geneRegions.add(new NonCodingExonicRegionViewModel(new BaseRegion(exon.Start, codingStart - 1), 0));
            }
            else
            {
                if(exon.End > codingEnd)
                    geneRegions.add(new NonCodingExonicRegionViewModel(new BaseRegion(codingEnd + 1, exon.End), 0));
            }
            ...
            while(codingRegion.containsPosition(pos))
            {
                ...
                geneRegions.add(new AminoAcidViewModel(region, aaPos, '.', '.', 0, false));
                if(phase == 0)
                    aaPos++;
            ...
            final TranscriptAminoAcids transcriptAminoAcids, final List<GeneRegionViewModel> geneRegions)
    {
        ...
        for(GeneRegionViewModel geneRegion : geneRegions)
        {
            ...
            if(geneRegion instanceof NonCodingExonicRegionViewModel)
            {
                viewModels.add(geneRegion);
                continue;
            }

            if(geneRegion instanceof AminoAcidViewModel)
            {
                AminoAcidViewModel aaRegion = (AminoAcidViewModel) geneRegion;
                int aaPos = aaRegion.aminoAcidPos();
                char refAcid = aminoAcids.charAt(aaPos - 1);
                viewModels.add(new AminoAcidViewModel(aaRegion.region(), aaPos, refAcid, '.', 0, aaPos == 1));
                continue;
            }
            ...
            final List<AminoAcidEvent> events, final BaseRegion renderRegion, final RefGenomeSource refGenome, final SageVariant variant)
    {
        ...
        for(int i = 0; i < geneRegions.size(); i++)
        {
            ...
            if(stopCoding)
            {
                viewModels.add(new NonCodingExonicRegionViewModel(geneRegion.region(), 0));
                continue;
            }
            ...
            if(geneRegion instanceof AminoAcidViewModel)
            {
                ...
                while(!eventQ.isEmpty() && eventQ.peek().region().start() <= aaPos)
                {
                    ...
                    else if(event instanceof AminoAcidDel)
                    {
                        ...
                        viewModels.add(new DelViewModel(new BaseRegion(delStart, delEnd)));
                        i = delEndIdx;
                        inDel = true;
                        break;
                    }
                    else if(event instanceof AminoAcidExt)
                    {
                        ...
                        if(posStrand)
                        {
                            ...
                            StringBuilder altBasesBuilder = new StringBuilder();
                            altBasesBuilder.append(refBases.substring(0, variantIdx));
                            altBasesBuilder.append(variant.alt());
                            altBasesBuilder.append(refBases.substring(variantIdx + variant.ref().length()));
                            String altBases = altBasesBuilder.toString();
                            ...
                            while(idx + 2 < altBases.length() && idx + 2 < refBases.length())
                            {
                                ...
                                char refAA = '.';
                                String codon;
                                if(geneRegionQ.peek() instanceof AminoAcidViewModel)
                                {
                                ...
                                {
                                    viewModels.add(new AminoAcidViewModel(
                            ...
                            if(stopCoding)
                            {
                                ...
                                if(pos <= renderRegion.end())
                                    viewModels.add(new NonCodingExonicRegionViewModel(new BaseRegion(pos, renderRegion.end()), 0));
                            }
                        }
                        else
                        {
                            ...
                            if(stopCoding)
                            {
                                ...
                                if(pos >= renderRegion.start())
                                    viewModels.add(new NonCodingExonicRegionViewModel(new BaseRegion(renderRegion.start(), pos), 0));
                            }
                        }
                ...
                if(!stopCoding || altAcid == STOP)
                    viewModels.add(new AminoAcidViewModel(aaRegion.region(), aaPos, refAcid, altAcid, 0, aaPos == 1));
                else
                    viewModels.add(new NonCodingExonicRegionViewModel(aaRegion.region(), 0));
    ...
    private static List<GeneRegionViewModel> realignViewModelsAroundDel(
            boolean posStrand, SageVariant variant, List<GeneRegionViewModel> viewModels)
    {
        ...
        for(GeneRegionViewModel viewModel : viewModels)
        {
            ...
            if(posStrand)
            {
                if(delRegion.start() == viewModel.region().start())
                {
                    ...
                    realignedViewModels.add(new DelViewModel(new BaseRegion(start, end)));
