...
public interface AminoAcidEvent
{
    ...
    static List<AminoAcidEvent> parse(final String hgvsStr)
    {
        ...
        if(TER_.equals(ref1.getLeft()))
        {
            ...
            triAA = matcher.group(1);
            if(TER_.equals(triAA))
                throw new IllegalArgumentException(format("hgvs(%s) does not follow expected format", hgvsStr));

            String aa = TRI_LETTER_AMINO_ACID_TO_SINGLE_LETTER.get(triAA);
            return List.of(new AminoAcidExt(ref1.getRight()));
        }

        String ref1AA = TRI_LETTER_AMINO_ACID_TO_SINGLE_LETTER.get(ref1.getLeft());
        String ref2AA = ref2 == null ? null : TRI_LETTER_AMINO_ACID_TO_SINGLE_LETTER.get(ref2.getLeft());
        ...
        // simple missense, including stop gained
        ...
        if(matcher.find())
        {
            ...
            String altAA = STOP.equals(triAltAA) ? STOP : TRI_LETTER_AMINO_ACID_TO_SINGLE_LETTER.get(triAltAA);
            return List.of(new AminoAcidSubstitution(ref1.getRight(), altAA.charAt(0)));
        }

        // start lost
        if(START_LOST_.contentEquals(hgvs))
        {
            if(ref2 != null)
                throw new IllegalArgumentException(format("hgvs(%s) does not follow expected format", hgvsStr));
        ...
        while(!hgvs.isEmpty())
        {
            ...
            if(STOP.contentEquals(hgvs))
            {
                altAcids.append(STOP);
                break;
            }
