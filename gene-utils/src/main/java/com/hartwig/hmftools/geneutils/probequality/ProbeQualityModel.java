...
public class ProbeQualityModel
{
            ...
            final int mMatchScoreOffset)
    {
        ...
        if (mTargetProbeLength < 1) {
            throw new RuntimeException("targetProbeLength must be >= 1");
        }
        ...
        if (mMatchScoreOffset < 0) {
            ...
            throw new RuntimeException("matchScoreOffset must be >= 0");
        }
    ...
    public List<Result> compute(final List<byte[]> probes) {
        probes.forEach(probe -> {
            if (probe.length != mTargetProbeLength) {
                throw new RuntimeException("Probe sequence length doesn't match configured probe length");
            }
        });
