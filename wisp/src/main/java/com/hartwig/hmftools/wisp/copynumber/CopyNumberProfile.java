package com.hartwig.hmftools.wisp.copynumber;
...
import static com.hartwig.hmftools.wisp.FileType.CN_PLOT_CALCS;
import static com.hartwig.hmftools.wisp.FileType.CN_SEGMENT;
...
public class CopyNumberProfile
{
    ...
    public CopyNumberProfile(final WispConfig config, final ResultsWriter resultsWriter, final SampleData sample)
    {
        mConfig = config;
    ...
    {
        for(int i = 0; i <= WispConstants.COPY_NUMBER_MAX; ++i)
        {
            if(abs(copyNumber - i) <= WispConstants.COPY_NUMBER_CLONAL_MARGIN)
                return true;
        }
    ...
    public static BufferedWriter initialiseCnRatioWriter(final WispConfig config)
    {
        try
        {
    ...
    public static BufferedWriter initialiseCnPlotCalcWriter(final WispConfig config)
    {
        try
        {
    ...
    private static synchronized void writePlotCalcData(
            final BufferedWriter writer, final WispConfig config, final SampleData sampleData, final String sampleId,
    ...
    public static boolean plotCopyNumberGcRatioFit(final String patientId, final String sampleId, final WispConfig config)
    {
        try
        {
