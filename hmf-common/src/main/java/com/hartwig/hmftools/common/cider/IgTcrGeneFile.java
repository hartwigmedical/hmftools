    ...
    public static List<IgTcrGene> read(RefGenomeVersion refGenomeVersion)
    {
        ...
        try(DelimFileReader reader = new DelimFileReader(new BufferedReader(new InputStreamReader(tsvStream))))
        {
            for(DelimFileReader.Row record : reader)
            {
                ...
                String chromosome = record.getOrNull(Column.contig);
                ...
                if(chromosome != null)
                {
                    ...
                    String anchorStart = record.getOrNull(Column.anchorStart);
                    String anchorEnd = record.getOrNull(Column.anchorEnd);
                ...
                igTcrGenes.add(new IgTcrGene(geneName, allele, region, functionality, genomicLocation, strand,
    ...
    public static void write(String tsvPath, List<IgTcrGene> geneList)
    {
        DelimFileWriter.write(tsvPath, Column.values(), geneList,
                (gene, row) ->
                {
                    ...
                    row.set(Column.contig, gene.geneLocation() != null ? gene.geneLocation().chromosome() : "");
