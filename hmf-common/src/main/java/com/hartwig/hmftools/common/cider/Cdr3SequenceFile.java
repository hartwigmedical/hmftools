    ...
    public static List<Cdr3Sequence> read(final String filename)
    {
        try (DelimFileReader reader = new DelimFileReader(filename))
        {
            return reader.stream().map(row -> ImmutableCdr3Sequence.builder()
                        .cdr3Seq(strOrEmpty(row.getOrNull(Column.cdr3Seq)))
                        .cdr3AA(strOrEmpty(row.getOrNull(Column.cdr3AA)))
                        .locus(strOrEmpty(row.getOrNull(Column.locus)))
                        .filter(strOrEmpty(row.getOrNull(Column.filter)))
                        .blastnStatus(strOrEmpty(row.getOrNull(Column.blastnStatus)))
