...
public class SagePostProcessVCF implements AutoCloseable, Consumer<VariantContext> {
    ...
    public SagePostProcessVCF(@NotNull final String outputVCF, @NotNull final List<DriverGene> driverGenes, final List<CanonicalTranscript> transcripts) {
        writer = new VariantContextWriterBuilder().setOutputFile(outputVCF).build();
        this.transcripts = transcripts;
        this.snpEffEnrichment = new SnpEffEnrichment(driverGenes, transcripts, writer::add);
    }
