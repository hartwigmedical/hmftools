...
public class JitterCountsTableConsensusFile
{
    private static String UNIT = "unit";
    public static String CONSENSUS_TYPE = "consensusType";
    ...
    public static void write(final String filename, @NotNull final Collection<JitterCountsTableConsensus> msStatsTables)
    {
        ...
        try(DelimFileWriter<JitterCountsTableConsensus.Row> writer = new DelimFileWriter<>(filename, columns, (msStatsTableRow, row) ->
        {
            row.set(UNIT,  msStatsTableRow.getRepeatUnit());
            row.set(CONSENSUS_TYPE, BqrReadType.NONE.name());
        ...
        {
            for(JitterCountsTableConsensus table : msStatsTables)
            {
                for(JitterCountsTableConsensus.Row row : table.getRows())
                {
    ...
    public static Collection<JitterCountsTableConsensus> read(final String filename) throws IOException
    {
        ...
        Collection<JitterCountsTableConsensus> jitterCountData = new ArrayList<>();

        DelimFileReader reader = new DelimFileReader(filename);

        for(DelimFileReader.Row row : reader)
        {
            String unit = row.get(JitterModelParamsConsensusFile.Column.unit);
            BqrReadType consensusType = stringConsensusMap.get(row.get(JitterModelParamsConsensusFile.Column.consensusType));
            JitterCountsTableConsensus unitData = null;
            for(JitterCountsTableConsensus perUnitData : jitterCountData)
            {
                if(perUnitData.RepeatUnit.equals(unit) && perUnitData.ConsensusType.equals(consensusType))
                {
            ...
            if(newUnit)
            {
                unitData = new JitterCountsTableConsensus(row.get(JitterModelParamsConsensusFile.Column.unit), 1.0,
                        stringConsensusMap.get(row.get(JitterModelParamsConsensusFile.Column.consensusType)));
            }
            JitterCountsTableConsensus.Row countRow = unitData.getOrCreateRow(row.getInt(NUM_UNITS));
