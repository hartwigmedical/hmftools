...
//        compareEvaluationMethods(d(5, 4, 1, 1), 0.0);  // todo investigate this and the line below - they should work
//        compareEvaluationMethods(d(5, 4, 1, 1), 50.0);
    ...
    private void compareEvaluationMethods(double[] data, double gamma) {
        Segmenter naive = new Segmenter(data, gamma, true);
        // Since we don't have direct access to leastCostSegmentation and segmentPenalty,
        // we'll compare the PCF representations instead
        PCF byAlgorithm = naive.pcf();
        PCF bySearch = naive.pcfByExhaustiveSearch();
        assertEquals(bySearch, byAlgorithm);
    }

    @Test
    public void allPossibleSegmentations() {
        Set<Segmentation> segmentations1 = new Segmenter(d(1)).allPossibleSegmentations();
        assertEquals(1, segmentations1.size());
        assertTrue(segmentations1.contains(segmentation(d(1))));

        Set<Segmentation> segmentations2 = new Segmenter(d(1, 1)).allPossibleSegmentations();
        ...
        Set<Segmentation> segmentations3 = new Segmenter(d(1, 2, 3)).allPossibleSegmentations();
        ...
        Set<Segmentation> segmentations4 = new Segmenter(d(1, 2, 3, 1)).allPossibleSegmentations();
    ...
    public void pcf() {
        assertEquals(
            new PCF(new int[]{1}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0), 50.0, false).pcf()
        );
        assertEquals(
            new PCF(new int[]{2}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0, 1.0), 50.0, false).pcf()
        );
        assertEquals(
            new PCF(new int[]{3}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0, 1.0, 1.0), 50.0, false).pcf()
        );

        assertEquals(
            new PCF(new int[]{3, 3}, new int[]{0, 3}, d(2.0, 12.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11), 50.0, false).pcf()
        );

        assertEquals(
            new PCF(new int[]{3, 3, 3}, new int[]{0, 3, 6}, d(2.0, 12.0, 2.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11, 2, 3, 1), 50.0, false).pcf()
        );
        assertEquals(
            new PCF(new int[]{3, 3, 3}, new int[]{0, 3, 6}, d(2.0, 12.0, 22.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11, 22, 23, 21), 50.0, false).pcf()
        );
    }

    @Test
    public void pcfExact() {
        assertEquals(
            new PCF(new int[]{1}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0), 50.0, false).pcfByExhaustiveSearch()
        );
        assertEquals(
            new PCF(new int[]{2}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0, 1.0), 50.0, false).pcfByExhaustiveSearch()
        );
        assertEquals(
            new PCF(new int[]{3}, new int[]{0}, d(1.0)),
            new Segmenter(d(1.0, 1.0, 1.0), 50.0, false).pcfByExhaustiveSearch()
        );

        assertEquals(
            new PCF(new int[]{3, 3}, new int[]{0, 3}, d(2.0, 12.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11), 50.0, false).pcfByExhaustiveSearch()
        );

        assertEquals(
            new PCF(new int[]{3, 3, 3}, new int[]{0, 3, 6}, d(2.0, 12.0, 2.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11, 2, 3, 1), 50.0, false).pcfByExhaustiveSearch()
        );
        assertEquals(
            new PCF(new int[]{3, 3, 3}, new int[]{0, 3, 6}, d(2.0, 12.0, 22.0)),
            new Segmenter(d(2, 3, 1, 12, 13, 11, 22, 23, 21), 50.0, false).pcfByExhaustiveSearch()
        );
    }
}
