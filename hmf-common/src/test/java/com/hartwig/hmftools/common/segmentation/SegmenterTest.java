...
public class SegmenterTest extends SegmentationTestBase
{
    ...
    private void compareEvaluationMethods(double[] data, double gamma)
    {
        ...
        double leastCostByExhaustiveSearch = segmenter.cheapestSegmentationByExhaustiveSearch().cost(segmentCost);
    ...
    public void pcfExact()
    {
        assertEquals(
                new PiecewiseConstantFit(new int[] { 1 }, new int[] { 0 }, d(1.0)),
                new Segmenter(d(1.0), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );
        assertEquals(
                new PiecewiseConstantFit(new int[] { 2 }, new int[] { 0 }, d(1.0)),
                new Segmenter(d(1.0, 1.0), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );
        assertEquals(
                new PiecewiseConstantFit(new int[] { 3 }, new int[] { 0 }, d(1.0)),
                new Segmenter(d(1.0, 1.0, 1.0), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );

        assertEquals(
                new PiecewiseConstantFit(new int[] { 3, 3 }, new int[] { 0, 3 }, d(2.0, 12.0)),
                new Segmenter(d(2, 3, 1, 12, 13, 11), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );

        assertEquals(
                new PiecewiseConstantFit(new int[] { 3, 3, 3 }, new int[] { 0, 3, 6 }, d(2.0, 12.0, 2.0)),
                new Segmenter(d(2, 3, 1, 12, 13, 11, 2, 3, 1), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );
        assertEquals(
                new PiecewiseConstantFit(new int[] { 3, 3, 3 }, new int[] { 0, 3, 6 }, d(2.0, 12.0, 22.0)),
                new Segmenter(d(2, 3, 1, 12, 13, 11, 22, 23, 21), 50.0, false).cheapestSegmentationByExhaustiveSearch().pcf()
        );
    }
}
