...
object IclusionTransvarReader : SomaticEventReader<IclusionEvent, HgvsAnnotation> {
    ...
    private fun match(event: IclusionEvent): Boolean {
        return event.types.size == 1 && event.types.contains(EventType.MUT) && TransvarMatcher.matches(event.variant)
    }

    private fun contains(event: IclusionEvent): Boolean {
        return event.types.size == 1 && event.types.contains(EventType.MUT) && TransvarMatcher.contains(event.variant)
    }
