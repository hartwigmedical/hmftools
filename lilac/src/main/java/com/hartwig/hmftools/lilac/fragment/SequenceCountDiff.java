...
public class SequenceCountDiff
{
    ...
    public static List<SequenceCountDiff> create(final SequenceCount referenceCount, final SequenceCount tumorCount)
    {
        ...
        loci.addAll(referenceCount.seqCountsByLoci_().keySet());
        loci.addAll(tumorCount.seqCountsByLoci_().keySet());

        for(int locus : loci)
        {
            boolean inRef = referenceCount.seqCountsByLoci_().containsKey(locus);
            boolean inTum = tumorCount.seqCountsByLoci_().containsKey(locus);
            Multiset<String> refSeqCounts = referenceCount.get_(locus);
            Multiset<String> tumorSeqCounts = tumorCount.get_(locus);
            if(inRef)
            {
