...
public class CoverageQC
{
    ...
    public static CoverageQC create(final Collection<Fragment> fragments, final ComplexCoverage winner)
    {
        ...
        for(HlaGene_ gene : GENE_CACHE.GeneNames)
        {
            ...
            countsByGene_.put(gene, count);
        }

        if(countsByGene_.values().stream().anyMatch(x -> x == 0))
        {
            StringJoiner msgBuilder = new StringJoiner(", ");
            countsByGene_.entrySet().stream().forEach(x -> msgBuilder.add(x.getValue() + " " + x.getKey().shortName() + " alleles"));
            LL_LOGGER.warn("  UNMATCHED ALLELE: {}", countsByGene_.toString());
        }
        ...
        return new CoverageQC(countsByGene_, totalFragments, unmatchedFragments, uninformativeFragments, hlaYFragments,
                winner.UniqueCoverage, winner.SharedCoverage, winner.WildCoverage);
    }

    public static List<String> header()
    {
        List<String> headerStrs = GENE_CACHE.GeneNames.stream().filter(x -> !x.isPseudo()).map(x -> x.shortName() + "Types").collect(Collectors.toCollection(Lists::newArrayList));
    ...
    {
        // TODO: String.valueOf(ATypes), String.valueOf(BTypes), String.valueOf(CTypes)
        List<String> bodyStrs = GENE_CACHE.GeneNames.stream()
                .filter(x -> !x.isPseudo())
                .map(gene -> String.valueOf(CountsByGene_.getOrDefault(gene, 0)))
