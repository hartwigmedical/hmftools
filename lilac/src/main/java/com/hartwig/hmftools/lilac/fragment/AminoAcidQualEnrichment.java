...
{
    public static List<Fragment> qualityFilterAminoAcidFragments(final LilacConfig config, final List<Fragment> fragments, int minEvidenceSupport_, double minEvidenceFactor_)
    {
        ...
        SequenceCount highQualityAminoAcidCounts = SequenceCount.aminoAcids_(minEvidenceSupport_, minEvidenceFactor_, qualityFilteredAminoAcidFragments);
    ...
    private static void applyQualFilter(final LilacConfig config, final Fragment fragment, final SequenceCount count)
    {
        ...
        fragment.aminoAcidsByLoci().values().stream()
                ...
                .forEach(locusIndex ->
                {
                    List<String> allowed = count.getMinEvidenceSequences_(locusIndex, config.MinEvidenceFactor_);
