...
public class ReferenceData
{
    ...
    public final List<HlaSequenceLoci> NucleotideSequences__;
    ...
    public final List<HlaSequenceLoci> HlaYNucleotideSequences_;
    public final List<HlaSequenceLoci> HlaYAminoAcidSequences_;
    ...
    public ReferenceData(final String resourceDir, final LilacConfig config)
    {
        ...
        if(config.Genes == HLA_DRB1)
            hlaTranscriptMap_ = trimDrb1Transcripts(hlaTranscriptMap_);
        ...
        for(HlaGene_ gene : GENE_CACHE.GeneNames)
            GENE_EXON_BOUNDARIES_.computeIfAbsent(gene, k -> Lists.newArrayList()).addAll(GENE_CACHE.AminoAcidExonBoundaries_.get(gene));
        ...
        NucleotideSequences__ = Lists.newArrayList();
        ...
        HlaYNucleotideSequences_ = config.Genes.coversMhcClass1() ? Lists.newArrayList() : null;
        HlaYAminoAcidSequences_ = config.Genes.coversMhcClass1() ? Lists.newArrayList() : null;
    ...
    private static Map<HlaGene_, TranscriptData> trimDrb1Transcripts(final Map<HlaGene_, TranscriptData> transcripts)
    {
        ...
        Map<HlaGene_, TranscriptData> trimmedTranscripts = Maps.newHashMap();
        trimmedTranscripts.put(HlaGene_.HLA_DRB1, trimmedTranscript);
    ...
    public static List<Integer> getAminoAcidExonBoundaries_(final HlaGene_ gene)
    {
        return GENE_CACHE.AminoAcidExonBoundaries_.get(gene);
    }

    public static List<Integer> getNucleotideExonBoundaries_(final HlaGene_ gene)
    {
        return GENE_CACHE.NucleotideExonBoundaries_.get(gene);
    }
    ...
    private void populateAminoAcidSequenceLookup()
    {
        for(HlaSequenceLoci seq : AminoAcidSequences__)
        {
            ...
            ExonSequencesLookup.computeIfAbsent(allele, k -> HlaExonSequences.create(GENE_CACHE.AminoAcidExonBoundaries_, seq));
        }
    }
    ...
    public boolean load()
    {
        ...
        if(!loadSequenceFile(nucleotideFilename, NucleotideSequences__, false))
            return false;
        ...
        if(HlaYNucleotideSequences_ != null)
        {
            HlaYNucleotideSequences_.addAll(NucleotideSequences__.stream().filter(x -> x.Allele.Gene == HLA_Y).toList());
            HlaYNucleotideSequences_.forEach(NucleotideSequences__::remove);
        }

        for(HlaSequenceLoci sequenceLoci : AminoAcidSequences__)
        {
    ...
    {
        if(HlaYAminoAcidSequences_ == null)
            return;

        // construct the AA allele sequences for HLA-Y from the nucleotides if it wasn't loaded
        HlaYAminoAcidSequences_.addAll(AminoAcidSequences__.stream().filter(x -> x.Allele.Gene == HLA_Y).toList());

        if(!HlaYAminoAcidSequences_.isEmpty())
        {
            HlaYAminoAcidSequences_.forEach(AminoAcidSequences__::remove);
        }
        else
        {
            for(HlaSequenceLoci sequenceLoci : HlaYNucleotideSequences_)
            {
                HlaYAminoAcidSequences_.add(buildAminoAcidSequenceFromNucleotides(sequenceLoci, mDeflatedSequenceTemplate));
            }
        }
    }
    ...
    private void markExonBoundaryWildcards()
    {
        for(HlaSequenceLoci sequence : AminoAcidSequences__)
        {
            ...
            List<Integer> exonBoundaries = getAminoAcidExonBoundaries_(sequence.Allele.Gene);
            sequence.setExonBoundaryWildcards(exonBoundaries);
        }

        for(HlaSequenceLoci sequence : NucleotideSequences__)
        {
            if(sequence.Allele.Gene.isPseudo())
                continue;

            List<Integer> exonBoundaries = getNucleotideExonBoundaries_(sequence.Allele.Gene);
            sequence.setExonBoundaryWildcards(exonBoundaries);
        }
    }
            ...
            final RefGenomeVersion refGenomeVersion, @Nullable final GeneSelector genes)
    {
        ...
        for(String line : hlaTranscriptData)
        {
            ...
            if(!geneId.equals(currentGene))
            {
                ...
                HlaGene_ gene = HlaGene_.fromString(geneName);
                if(genes == null || genes.contains(gene))
                    hlaTranscriptMap_.put(gene, currentTrans);
        ...
        return hlaTranscriptMap_;
    }

    private boolean loadSequenceFile(final String filename, final List<HlaSequenceLoci> sequenceData_, boolean isProteinFile)
    {
        try
        {
            final List<String> fileContents = Files.readAllLines(new File(filename).toPath());
            fileContents.remove(0); // remove header
            loadSequenceFile(fileContents, sequenceData_, isProteinFile);
    ...
    public void loadSequenceFile(final Iterable<String> fileContents, final Collection<HlaSequenceLoci> sequenceData_, boolean isProteinFile)
    {
        for(String line : fileContents)
        {
            ...
            {
                int nucCount = GENE_CACHE.NucleotideLengths.get(HlaGene_.HLA_DRB1);
                int acidCount = nucCount / 3;
                int trimLength = isProteinFile ? acidCount : nucCount;
                List<String> trimmedSequence = Lists.newArrayList(newSequence.getSequences().subList(0, min(trimLength, newSequence.getSequences().size())));
