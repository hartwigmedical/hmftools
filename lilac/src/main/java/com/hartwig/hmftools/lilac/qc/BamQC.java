...
public class BamQC
{
    ...
    public final Map<HlaGene_, Integer> GeneLowCoverageCounts;

    private static final int MIN_SUPPORT = 3;

    public BamQC(int discardedAlignmentFragments, int discardedIndels, int discardedIndelMaxFrags, final Map<HlaGene_, int[]> geneBaseDepth)
    {
    ...
    public List<String> body()
    {
        ...
        GENE_CACHE.GeneNames.stream().filter(x -> !x.isPseudo()).forEach(gene -> bodyStrs.add(String.valueOf(GeneLowCoverageCounts.get(gene))));
        return bodyStrs;
    }

    public static BamQC create(final BamReader reader, final Map<HlaGene_, int[]> geneBaseDepth)
    {
