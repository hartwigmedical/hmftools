...
[oncoanalyser](https://github.com/scwatts/oncoanalyser), a Nextflow implementation of the HMF pipeline.
...
[GRCh37](https://console.cloud.google.com/storage/browser/hmf-public/HMFtools-Resources/ref_genome/37) or [GRCh38](https://console.cloud.google.com/storage/browser/hmf-public/HMFtools-Resources/ref_genome/38) reference genomes.
...
| Name              | Tumor DNA BAM      | Normal DNA BAM     | Tumor RNA BAM      |
| ---               | :-:                | :-:                | :-:                |
...
| Name       | Tumor DNA BAM      | Tumor RNA BAM |
| ---        | :-:                | :-:           |
| Tumor only | :white_check_mark: | *optional*    |

## Installation

No installation is required but please ensure that all [software requirements](#software-requirements) are met prior
running Nextflow.
...
> [!NOTE]
> Docker on Windows and macOS can perform poorly, so only running oncoanalyser Linux is currently recommended.
...
so the full tumor/normal WGTS workflow will be run (see [WGTS workflow inputs](#available-modes) and [targeted workflow
inputs](#available-modes-1) for more information).
...
COLO829_example,COLO829,COLO829R,tumor,rna,bam,/path/to/COLO829T.rna.bam
COLO829_example,COLO829,COLO829R,normal,dna,bam,/path/to/COLO829R.dna.bam
SEQC_example,SEQC,SEQCT,tumor,dna,bam,/path/to/SEQCR.dna.bam
```

Here the `SEQC_example` has been added to the previous example. Since only a tumor DNA BAM is provided for this
additional group, just the tumor-only WGS workflow is run.
...
| filepath      | Absolute filepath to input file (can local filepath, URL, S3 URI) |
...
directories named by the respective `group_id` value from the input samplesheet. Within each group directory, outputs
are also organised by tool.

All intermediate files used by each process are also kept in the Nextflow work directory (default: `work/`). Once
analysis has completed this directory can be removed.

### Sample reports

| Report | Path                                             | Description                                     |
| ---    | ---                                              | ---                                             |
| ORANGE | `<group_id>/orange/<tumor_sample_id>.orange.pdf` | PDF summary report of key HMF pipeline findings |
