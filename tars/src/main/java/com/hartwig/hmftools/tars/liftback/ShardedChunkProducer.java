...
public class ShardedChunkProducer extends Thread
{
    ...
    public void run()
    {
        try
        {
            ...
            List<BamShardSplitter.ShardRange> ranges = BamShardSplitter.computeSplits(bam, header, mShardCount);
            ...
            final AtomicBoolean done)
    {
        ...
        while(!done.get())
        {
            ...
            double percent = fileLength > 0 ? Math.min(100.0, consumed * 100.0 / fileLength) : 0.0;
            long reads = readsCounter.sum();
            long nowNanos = System.nanoTime();
            double intervalSecs = (nowNanos - lastNanos) / 1e9;
    ...
    {
        int filled = Math.min(PROGRESS_BAR_WIDTH, percent * PROGRESS_BAR_WIDTH / 100);
