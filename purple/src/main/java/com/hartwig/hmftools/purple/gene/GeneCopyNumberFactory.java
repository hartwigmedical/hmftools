...
public final class GeneCopyNumberFactory
{
    ...
    public static List<GeneCopyNumber> geneCopyNumbers(
            @NotNull final List<HmfTranscriptRegion> genes,
            @NotNull final List<PurpleCopyNumber> somaticCopyNumbers, @NotNull final List<PurpleCopyNumber> germlineDeletions)
    {
        ...
        for(HmfTranscriptRegion gene : genes)
        {
            ...
            RegionZipper.zip(somaticCopyNumbers, gene.exons(), builder);
            RegionZipper.zip(germlineDeletions, gene.exons(), builder);
