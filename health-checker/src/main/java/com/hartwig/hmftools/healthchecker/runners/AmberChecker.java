...
public class AmberChecker implements HealthChecker {
    ...
    public List<QCValue> run() throws IOException {
        AmberQC qcCheck = AmberQCFile.read(AmberQCFile.generateFilename(amberDirectory, tumorSample));

        QCValue bafCheck = ImmutableQCValue.of(QCValueType.AMBER_MEAN_BAF, String.valueOf(qcCheck.meanBAF()));
        QCValue contaminationCheck = ImmutableQCValue.of(QCValueType.AMBER_CONTAMINATION, String.valueOf(qcCheck.contamination()));
