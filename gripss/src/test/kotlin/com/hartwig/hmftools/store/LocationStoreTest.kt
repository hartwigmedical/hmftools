...
class LocationStoreTest {
    ...
    fun testContainSingle() {
        ...
        val store = LocationStore(listOf(entry1, entry2), listOf())
    ...
    fun testOutOfOrder() {
        val entry1 = Breakend.fromBed("1\t665\t666\t.\t440\t+")
        val entry2 = Breakend.fromBed("1\t766\t767\t.\t440\t+")
        val store = LocationStore(listOf(entry2, entry1), listOf())
        Assert.assertTrue(store.contains(Breakend("1", 666, 666, 1)))
        Assert.assertFalse(store.contains(Breakend("1", 667, 667, 1)))
        Assert.assertTrue(store.contains(Breakend("1", 666, 666, 1)))
    }
    ...
    fun testGoBackFarEnough() {
        ...
        val store = LocationStore(listOf(), listOf(entry2, entry1, entry3))
    ...
    fun testStuff() {
        ...
        val breakpoints = Breakpoint.fromBedpeFile("/Users/jon/hmf/resources/gridss_pon_breakpoint.bedpe")

        val ponStore = LocationStore(breakEnds, breakpoints)
