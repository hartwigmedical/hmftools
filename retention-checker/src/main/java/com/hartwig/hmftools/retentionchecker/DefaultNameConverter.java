...
class DefaultNameConverter implements FileNameConverter {
    ...
    public String apply(@NotNull String originalFileName) {
        ...
        final String nameWithoutExtension = originalFileName.substring(0, originalFileName.indexOf("."));

        final String splitRegExp = "_";
        final String[] parts = nameWithoutExtension.split(splitRegExp);
        if (parts[5].length() == 1) {
            // KODU: We have converted fastq as input
            return nameWithoutExtension + ".fastq";
        } else {
            final String readGroup = parts[4].equals("R1") ? "1" : "2";
