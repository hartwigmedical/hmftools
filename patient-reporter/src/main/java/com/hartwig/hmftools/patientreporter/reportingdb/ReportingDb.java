...
public final class ReportingDb {
    ...
    public static void addSequenceReportToReportingDb(@NotNull String reportingDbTsv, @NotNull AnalysedPatientReport report)
        ...
        if (sampleId.startsWith("COLO")) {
        ...
        } else if (type.equals(LimsSampleType.CORE) && report.clinicalSummary().isEmpty()) {
            if (sampleId.startsWith("CORELR") || sampleId.startsWith("CORERI")) {
                checkAddToFile(reportingDbTsv,
                        sampleId,
                        tumorBarcode,
                        reportType,
                        reportDate,
            ...
            checkAddToFile(reportingDbTsv, sampleId, tumorBarcode, reportType, reportDate, purity, hasReliablePurity, hasReliableQuality);

        }
    }

    private static void checkAddToFile(@NotNull String reportingDbTsv, @NotNull String sampleId, @NotNull String tumorBarcode,
