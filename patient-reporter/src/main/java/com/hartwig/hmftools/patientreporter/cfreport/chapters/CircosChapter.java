...
public class CircosChapter extends ReportChapter {
    ...
    protected void renderChapterContent(Document report) {
        ...
        table.addCell(TableHelper.getLayoutCell().add(new Div()
            .add(getContentDiv("The third circle", " shows all observed tumor purity adjusted copy number changes,including both focal and chromosomal somatic events. Copy number losses are indicated in red, green shows regions of copy number gain. The scale ranges from 0 (complete loss) to 6 (high level gains). If the absolute copy number is > 6 it is shown as 6 with a green dot on the diagram.")
            .add(getContentDiv("The fourth circle", " represents the observed 'minor allele copy numbers’ across the chromosome. The range of the chart is from 0 to 3. The expected normal minor allele copy number is 1, and anything below 1 is shown as a loss (orange) and represents a LOH event. Minor allele copy numbers above 1 (blue) indicate amplification events of both A and B alleles at the indicated locations.")))));

        table.addCell(TableHelper.getLayoutCell().add(new Div()
                .add(getContentDiv("The innermost circle", " displays the observed structural variants within or between the chromosomes. Translocations are indicated in blue, deletions in red, insertions in yellow, tandem duplications in green and inversions in black."))));
    ...
    @NotNull
    private final static Paragraph getContentDiv(@NotNull String boldPart, @NotNull String regularPart) {
