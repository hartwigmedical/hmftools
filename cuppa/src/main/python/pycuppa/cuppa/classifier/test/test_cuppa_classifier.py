...
class TestMissingFeaturesHandler:
    ...
    def test_fill_when_some_samples_lack_rna_features(self):
        X = pd.DataFrame(
            [
                [1, 1, 1, 1, 1],
                [1, 1, 1, 1, np.nan],
        ...
        assert X_filled.loc["sample_1", "gene_exp.feat_2"] == 0
        assert X_filled.loc["sample_2", required_rna_features].isna().all()
