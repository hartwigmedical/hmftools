...
{
    CandidatePeak mLevel1 = new CandidatePeak(0.1);

    @Before
    public void setup()
    {
        mLevel1 = new CandidatePeak(0.1);
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 1000, 100, 90));
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 2000, 100, 91));
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 3000, 100, 10));
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 4000, 100, 9));
    }
    ...
    public void highAlleleDepthVariantsAreSkewedTest()
    {
        ...
        PeakGnomadFrequenciesChecker classifier = new PeakGnomadFrequenciesChecker(mLevel1);
    ...
    {
        mLevel1 = new CandidatePeak(0.1);
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 1000, 1000, 900)); // hom
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 2000, 1000, 50)); // het
        GnomadFrequencySupplier supplier = (chromosome, position) ->
        {
            if(position < 1500)
            {
    ...
    {
        mLevel1 = new CandidatePeak(0.1);
        mLevel1.test(evidenceWithDepthAndAltCount(_1, 2000, 1000, 950)); // het
        GnomadFrequencySupplier supplier = (chromosome, position) -> 0.5;
        PeakGnomadFrequenciesChecker classifier = new PeakGnomadFrequenciesChecker(mLevel1);
        assertFalse(classifier.checkGnomadFrequencies(supplier, 0.5));
    }
