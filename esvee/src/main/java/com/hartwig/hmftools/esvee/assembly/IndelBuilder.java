...
public final class IndelBuilder
{
    ...
    public static void findIndelExtensions(final JunctionAssembly assembly, final List<Read> unfilteredNonJunctionReads)
    {
        ...
        for(Read read : unfilteredNonJunctionReads)
        {
            ...
            if(positionsOverlap(
                    indelCoords.PosStart + 1, indelCoords.PosEnd - 1, read.alignmentStart(), read.alignmentEnd()))
            {
                continue;
            }
            ...
            if(!isJunctionMate)
            {
                ...
                if(positionsOverlap(
                        indelCoords.PosStart + 1, indelCoords.PosEnd - 1, read.mateAlignmentStart(), read.mateAlignmentEnd()))
                {
                    continue;
                }
    ...
    public static String findInsertedBases(final Read read)
    {
        ...
        {
            if(refPosition == indelCoords.PosStart)
            {
                StringBuilder insertedBases = new StringBuilder();

                for(int i = 0; i < element.getLength(); ++i)
                {
                    insertedBases.append(read.getBases()[readIndex + i]);
                }
