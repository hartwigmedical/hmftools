...
import static com.hartwig.hmftools.esvee.pon_gen.PonLocations.formSglLocationId;
...
public class PonStore
{
    ...
    {
        PonLocations locations = getOrAddSvLocation(PonLocations.formSvLocationId(chrStart, chrEnd, orientStart, orientEnd));
        locations.addPosition(posStart, posEnd);
    }

    public void addSgl(final String chromosome, final byte orientation, final int position)
    {
        PonLocations locations = getOrAddSglLocation(formSglLocationId(chromosome, orientation));
    ...
    {
        return mSvLocations.get(PonLocations.formSvLocationId(chrStart, chrEnd, orientStart, orientEnd));
    }

    public PonLocations getLocation(final String chromosome, final byte orientation)
    {
        return mSglLocations.get(formSglLocationId(chromosome, orientation));
    }

    public LocationCounter getLocationCounter(
            final String chrStart, final String chrEnd, final byte orientStart, final byte orientEnd, final int posStart, final int posEnd)
    {
        PonLocations locations = mSvLocations.get(PonLocations.formSvLocationId(chrStart, chrEnd, orientStart, orientEnd));
        if(locations == null)
            return null;
    ...
    {
        PonLocations locations = mSglLocations.get(formSglLocationId(chromosome, orientation));
        if(locations == null)
            return null;
