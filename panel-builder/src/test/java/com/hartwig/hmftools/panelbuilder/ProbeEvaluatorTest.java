...
public class ProbeEvaluatorTest
{
    ...
    {
        Probe probe = new Probe(new ChrBaseRegion("1", 1, 10), "ACGTACGTAC", METADATA, null, null, 1, 0.5);
    ...
    {
        Probe probe = new Probe(new ChrBaseRegion("1", 11, 20), "AAAAAAAAAA", METADATA, null, null, 1, 0);
        Probe evalProbe = mProbeEvaluator.evaluateProbe(probe, CRITERIA);
        assertNotSame(probe, evalProbe);
        assertEquals(probe.metadata(), evalProbe.metadata());
        assertEquals(CRITERIA, evalProbe.evalCriteria());
    ...
    {
        Probe probe = new Probe(new ChrBaseRegion("1", 21, 30), "ACGTACGTAC", METADATA, null, null, 0.1, 0.5);
        Probe evalProbe = mProbeEvaluator.evaluateProbe(probe, CRITERIA);
        assertNotSame(probe, evalProbe);
        assertEquals(probe.metadata(), evalProbe.metadata());
        assertEquals(CRITERIA, evalProbe.evalCriteria());
