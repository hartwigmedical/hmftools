...
public class PatientValidator {
    ...
    static List<ValidationFinding> validatePatientData(@NotNull final PatientData patientData) {
        ...
        if (cpctId != null) {
            ...
            if (patientData.gender() == null) {
                findings.add(new ImmutableValidationFinding(cpctId, FIELD_SEX, "gender empty", patientData.demographyStatus(),
                        patientData.demographyLocked()));
            }
            if (patientData.registrationDate() == null) {
                findings.add(new ImmutableValidationFinding(cpctId, FIELD_REGISTRATION_DATE2, "registration date empty or in wrong format",
                        patientData.selectionCriteriaStatus(), patientData.selectionCriteriaLocked()));
                findings.add(new ImmutableValidationFinding(cpctId, FIELD_REGISTRATION_DATE1, "registration date empty or in wrong format",
                        patientData.eligibilityStatus(), patientData.eligibilityLocked()));
            }
            if (patientData.birthYear() == null) {
                findings.add(new ImmutableValidationFinding(cpctId, FIELD_BIRTH_YEAR1, "birth year could not be determined",
                        patientData.selectionCriteriaStatus(), patientData.selectionCriteriaLocked()));
                findings.add(new ImmutableValidationFinding(cpctId, fields(FIELD_BIRTH_YEAR2, FIELD_BIRTH_YEAR3),
            ...
            if (patientData.ethnicity() == null) {
                findings.add(new ImmutableValidationFinding(cpctId, FIELD_ETHNICITY, "ethnicity empty", patientData.demographyStatus(),
                        patientData.demographyLocked()));
            }
        }
        return findings;
    ...
    static List<ValidationFinding> validateBiopsies(@NotNull final String patientId, @NotNull final List<BiopsyData> biopsies,
        ...
        if (biopsies.size() > 0 && treatments.size() > 0) {
            ...
            if (firstBiopsyDate != null && firstTreatmentStart != null && firstTreatmentStart.isBefore(firstBiopsyDate)) {
                findings.add(new ImmutableValidationFinding(patientId, FORM_TREATMENT, "first treatment start is before first biopsy date",
                        biopsies.get(0).formStatus(), biopsies.get(0).formLocked()));
                findings.add(new ImmutableValidationFinding(patientId, FORM_BIOPS, "first treatment start is before first biopsy date",
        ...
        if (treatments.size() > 1) {
            ...
            if (nonFinishedTreatments.size() > 1) {
                nonFinishedTreatments.forEach(treatment -> findings.add(
                        new ImmutableValidationFinding(patientId, FORM_TREATMENT, "end of at least 1 non-final treatment is missing",
    ...
    static List<ValidationFinding> validateTreatmentData(@NotNull final String patientId,
        ...
        if (treatmentGiven == null) {
            ...
            if (treatmentData.drugs().isEmpty()) {
                findings.add(
                        new ImmutableValidationFinding(patientId, FORM_TREATMENT, "treatment given is yes, but no treatment data filled in",
                                treatmentData.formStatus(), treatmentData.formLocked()));
            } else {
                treatmentData.drugs()
    ...
    static List<ValidationFinding> validateDrugData(@NotNull final String patientId, @NotNull final BiopsyTreatmentDrugData drugData,
        ...
        final List<ValidationFinding> findings = Lists.newArrayList();
        if (drugStart == null) {
            findings.add(new ImmutableValidationFinding(patientId, FIELD_DRUG_START, "drug start date empty or in wrong format", formStatus,
                    formLocked));
        } else {
        ...
        if (drugData.name() == null) {
            findings.add(new ImmutableValidationFinding(patientId, fields(FIELD_DRUG, FIELD_DRUG_OTHER), "drug name empty", formStatus,
                    formLocked));
        }
        return findings;
    }
    ...
    static List<ValidationFinding> validateDeathDate(@NotNull final String patientId, @NotNull final PatientData patient,
        ...
        if (deathDate != null && !treatments.isEmpty()) {
            ...
            if (lastTreatmentEndDate == null || lastTreatmentEndDate.isAfter(deathDate)) {
                findings.add(new ImmutableValidationFinding(patientId, FIELD_DEATH_DATE, "death date before end of last treatment",
                        patient.deathStatus(), patient.deathLocked()));
                findings.add(new ImmutableValidationFinding(patientId, FORM_TREATMENT, "death date before end of last treatment",
    ...
    static List<ValidationFinding> validateRegistrationDate(@NotNull final String patientId, @NotNull final PatientData patient,
        ...
        if (registrationDate != null && !biopsies.isEmpty()) {
            ...
            if (biopsiesPriorToRegistration.size() > 0) {
                findings.add(new ImmutableValidationFinding(patientId, FIELD_REGISTRATION_DATE2,
                        "at least 1 biopsy date prior to registration date", patient.selectionCriteriaStatus(),
                        patient.selectionCriteriaLocked()));
                findings.add(new ImmutableValidationFinding(patientId, FIELD_REGISTRATION_DATE1,
                ...
                biopsiesPriorToRegistration.forEach(biopsy -> findings.add(
                        new ImmutableValidationFinding(patientId, FIELD_BIOPSY_DATE, "at least 1 biopsy date prior to registration date",
