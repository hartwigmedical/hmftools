...
class GeneCopyNumberDAO {
    ...
    void writeCopyNumber(@NotNull final String sample, @NotNull List<GeneCopyNumber> copyNumbers) {
        ...
        for (List<GeneCopyNumber> splitCopyNumbers : Iterables.partition(copyNumbers, BATCH_INSERT_SIZE)) {
            InsertValuesStep13 inserter = context.insertInto(GENECOPYNUMBER, GENECOPYNUMBER.SAMPLEID,
                    GENECOPYNUMBER.CHROMOSOME, GENECOPYNUMBER.START, GENECOPYNUMBER.END, GENECOPYNUMBER.GENE,
                    GENECOPYNUMBER.MINCOPYNUMBER, GENECOPYNUMBER.MAXCOPYNUMBER, GENECOPYNUMBER.MEANCOPYNUMBER,
                    GENECOPYNUMBER.REGIONS, GENECOPYNUMBER.TRANSCRIPTID, GENECOPYNUMBER.TRANSCRIPTVERSION,
    ...
    private void addCopynumberRecord(Timestamp timestamp, InsertValuesStep13 inserter, String sample,
            GeneCopyNumber gene) {
        inserter.values(sample, gene.chromosome(), gene.start(), gene.end(), gene.gene(), gene.minCopyNumber(),
                gene.maxCopyNumber(), gene.meanCopyNumber(), gene.regions(), gene.transcriptID(),
