...
public class DepthTaskOld implements Callable
{
    ...
    public DepthTaskOld(final String chromosome, final DepthConfig config, final Map<String,Integer> sampleVcfGenotypeIds)
    {
        mConfig = config;
    ...
    private void retrieveDepth(final List<VariantContext> variants, int posStart, int posEnd)
    {
        ...
        if(mConfig.PerfLogTime > 0 &&  mPerfCounter.getLastTime() > mConfig.PerfLogTime)
        {
            ...
            SV_LOGGER.debug("chr({}) span({}-{}) variants({}) high depth retrieval time({}) totalFrags({}) times({}) readCounts({})",
    ...
    private void calculateVariantSupport(final VariantContext variant, final String sampleId, final RefSupportCounts totalCounts)
    {
