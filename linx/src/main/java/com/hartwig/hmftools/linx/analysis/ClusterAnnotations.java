...
import com.hartwig.hmftools.linx.types.SvaConstants;
...
public class ClusterAnnotations
{
    ...
    public static void annotateTemplatedInsertions(final List<SvCluster> clusters, final Map<String, List<SvBreakend>> chrBreakendMap)
    {
        ...
        for(final SvCluster cluster : clusters)
        {
            ...
            for(final SvChain chain : cluster.getChains())
            {
                ...
                {
                    final SvBreakend chainEnd = chain.getOpenBreakend(SvVarData.isStart(be1));
                    if (chainEnd == null)
                        continue;
                ...
                for(final SvLinkedPair pair : chain.getLinkedPairs())
                {
                    ...
                    if(startEndSameArm)
                    {
                        if(pairLowerBe.chromosome().equals(chainLowerBe.chromosome()))
                        {
                            ...
                            {
                                pair.setLocationType(SvLinkedPair.LOCATION_TYPE_INTERNAL);
                            }
                            else
                            {
                                pair.setLocationType(SvLinkedPair.LOCATION_TYPE_EXTERNAL);
                            }
                        }
                        else
                        {
                            pair.setLocationType(SvLinkedPair.LOCATION_TYPE_REMOTE);
                        }
                    }
                    else
                    {
                    ...
                    for(final SvLinkedPair otherPair : chain.getLinkedPairs())
                    {
                        ...
                        if(overlapDistance >= Math.abs(SvaConstants.NO_DB_MARKER)) // longer than a max DB length
                        {
                            ++overlapCount;
                        }
                    }
                    ...
                    pair.setDBLenFirst(dbFirst != null ? dbFirst.length() : SvaConstants.NO_DB_MARKER);
                    pair.setDBLenSecond(dbSecond != null ? dbSecond.length() : SvaConstants.NO_DB_MARKER);
    ...
    public static void annotateChainedClusters(final SvCluster cluster, long proximityCutoff)
    {
        ...
        for (final SvChain chain : cluster.getChains())
        {
            ...
            for(final SvLinkedPair pair : chain.getLinkedPairs())
            {
                ...
                if((pair.getDBLenFirst() > SvaConstants.NO_DB_MARKER && pair.getDBLenFirst() <= proximityCutoff)
                || (pair.getDBLenSecond() > SvaConstants.NO_DB_MARKER && pair.getDBLenSecond() <= proximityCutoff))
                {
                    ++armData[CHAIN_TI_DB_COUNT];
                }

                if(pair.length() <= SvaConstants.SHORT_TI_LENGTH)
                {
        ...
        if(isComplete)
        {
            ...
            if(!isComplex)
            {
                cluster.addAnnotation(SvCluster.CLUSTER_ANNONTATION_CT);
            }
        }
    }
            ...
            final Map<String, List<SvBreakend>> chrBreakendMap, final CnDataLoader cnAnalyser)
    {
        ...
        for(SvArmGroup armGroup : cluster.getArmGroups())
        {
            ...
            for (SvVarData var : clusterFoldbacks)
            {
                ...
                maxFoldbackPloidy = Math.max(maxFoldbackPloidy, breakend.getSV().ploidyMin());
                ++foldbackCount;
            }
        }
        ...
        for(Map.Entry<SvArmGroup, List<SvVarData>> entry : armGroupFoldbacks.entrySet())
        {
            ...
            for (SvVarData var : foldbacks)
            {
                ...
                double ploidy = direction == SvBreakend.DIRECTION_CENTROMERE ? Math.abs(var.ploidyMin()) : -Math.abs(var.ploidyMin());
            ...
            for (int i = 0; i <= 1; ++i)
            {
                ...
                while (true)
                {
                    ...
                    for (final SvBreakend otherBreakend : nextClusterBreakends)
                    {
                        ...
                        {
                            maxOpposingPloidy = Math.max(maxOpposingPloidy, otherBreakend.getSV().ploidyMin());
                        }
            ...
            infoStr += String.format(",%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",
                    telomereCN, telomereMAP, centromereCNData[CnDataLoader.CN_SEG_DATA_CN_BEFORE], centromereCNData[CnDataLoader.CN_SEG_DATA_MAP_BEFORE],
                    centromereCNData[CnDataLoader.CN_SEG_DATA_CN_AFTER], centromereCNData[CnDataLoader.CN_SEG_DATA_MAP_AFTER]);
            ...
            boolean breakOnOtherCluster)
    {
        ...
        for(int i = 0; i <= 1; ++i) // first forwards, then backwards through the breakends
        {
            ...
            while(index >= 0 && index < breakendList.size())
            {
                ...
                if(breakOnOtherCluster && prevFullListIndex > -1 && Math.abs(breakend.getChrPosIndex() - prevFullListIndex) > 1)
                    break;
        ...
        */
        ...
        for (final Map.Entry<String, List<SvBreakend>> entry : chrBreakendMap.entrySet())
        {
            ...
            for(final SvBreakend breakend : breakendList)
            {
                ...
                SvVarData.isSpecificSV(breakend.getSV());
                ...
                else
                {
                    ...
                    minDMCopyNumber = Math.min(breakend.copyNumber(), minDMCopyNumber);
                    maxDMCopyNumber = Math.max(breakend.copyNumber(), maxDMCopyNumber);
            ...
            int overlappedCount, final CnDataLoader cnAnalyser, final SvGeneTranscriptCollection geneCollection)
    {
        ...
        for(final SvVarData var : dmSVList)
        {
            ...
            maxDMCopyNumber = Math.max(maxDMCopyNumber, Math.max(var.copyNumber(true), var.copyNumber(false)));
            ...
            if (!clusters.contains(cluster))
            {
                ...
                cluster.addAnnotation(SvCluster.CLUSTER_ANNONTATION_DM);
    ...
    private static final SvChain createDMChain(List<SvVarData> dmSVList)
    {
        if(dmSVList.size() == 1)
        {
            ...
            SvLinkedPair pair = new SvLinkedPair(var, var, SvLinkedPair.LINK_TYPE_TI, true, false);
    ...
    public static void reportDoubleMinutes(final SvCluster cluster, final Map<String, List<SvBreakend>> chrBreakendMap)
    {
        ...
        for(int i = 0; i < ploidyList.size(); ++i)
        {
            ...
            for(int be = SvVarData.SE_START; be <= SvVarData.SE_END; ++be)
            {
                boolean useStart = SvVarData.isStart(be);
        ...
        if(isPotentialDM)
        {
            ...
            final String dmAnnotation = otherClustersHaveHighPloidy ? SvCluster.CLUSTER_ANNONTATION_DM + "_Unclear" : SvCluster.CLUSTER_ANNONTATION_DM;
        ...
        for(SvArmGroup armGroup : cluster.getArmGroups())
        {
            ...
            for (final SvArmCluster armCluster : potentialRepRepairGroups)
            {
                ...
                LOGGER.log(logLevel,
                        ...
                        armGroup.toString(), SvArmCluster.typeToString(armCluster.getType()),
    ...
    private static List<SvLinkedPair> formPossibleLinkedPairsByShortest(List<SvBreakend> unlinkedBreakends)
    {
        ...
        while(!unlinkedBreakends.isEmpty())
        {
            ...
            for(int i = 0; i < unlinkedBreakends.size() - 2; ++i)
            {
                ...
                SvLinkedPair pair = SvLinkedPair.from(be1, be2, SvLinkedPair.LINK_TYPE_TI);
                pairs.add(pair);
            ...
            final Map<String, List<SvBreakend>> chrBreakendMap)
    {
        ...
        while(index < unmatchedLohList.size())
        {
            ...
            for(int i = 0; i < breakendList.size(); ++i)
            {
                ...
                for (int be1 = SvVarData.SE_START; be1 <= SvVarData.SE_END; ++be1)
                {
                    boolean v1Start = SvVarData.isStart(be1);
    ...
    public static void findChainRepeatedSegments(final String sampleId, final SvCluster cluster, final SvChain chain)
    {
        ...
        for (int i = 0; i < svList.size(); ++i)
        {
            ...
            for (int j = i + 1; j < svList.size(); ++j)
            {
                ...
                List<SvVarData> forwardRepeats = SvChain.getRepeatedSvSequence(svList, i, j, true);
                ...
                else
                {
                    ...
                    forwardRepeats = SvChain.getRepeatedSvSequence(svList, i, j, false);
                }
