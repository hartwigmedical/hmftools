...
public class SvSampleAnalyser {
    ...
    private void checkPseudoGeneAnnotations(SvGeneTranscriptCollection geneTransCache)
    {
        for(final SvCluster cluster : getClusters())
        {
            ...
            {
                if(pair.length() > SvaConstants.SHORT_TI_LENGTH * 8)
                    continue;
                ...
                for(final GeneAnnotation gene1 : lower.getSV().getGenesList(lower.usesStart()))
                {
                    for(final GeneAnnotation gene2 : upper.getSV().getGenesList(upper.usesStart()))
                    {
                        ...
                        if(exonData[SvGeneTranscriptCollection.PSEUDO_GENE_DATA_TRANS_ID] != null)
                        {
                            pseudoGene = gene1;
                            transcriptId = exonData[SvGeneTranscriptCollection.PSEUDO_GENE_DATA_TRANS_ID];

                            String exonMatchData = String.format("%s:%s;%s",
                                    gene1.GeneName, transcriptId, exonData[SvGeneTranscriptCollection.PSEUDO_GENE_DATA_EXON_RANK]);
        ...
        try
        {
            for(final SvVarData var : mAllVariants)
            {
                ...
                if(mSvFileWriter != null)
                {
                    ...
                    int dbLenStart = var.getDBLink(true) != null ? var.getDBLink(true).length() : SvaConstants.NO_DB_MARKER;
                    int dbLenEnd = var.getDBLink(false) != null ? var.getDBLink(false).length() : SvaConstants.NO_DB_MARKER;
                    ...
                    for (int be = SvVarData.SE_START; be <= SvVarData.SE_END; ++be)
                    {
                        SvArmCluster armCluster = be == SvVarData.SE_START ? armClusterStart : armClusterEnd;
        ...
        try
        {
            for(final SvCluster cluster : getClusters())
            {
                ...
                if(mClusterFileWriter != null)
                {
                    ...
                    long shortTIs = cluster.getLinkedPairs().stream().filter(x -> x.length() <= SvaConstants.SHORT_TI_LENGTH).count();
                    ...
                    mClusterFileWriter.write(String.format(",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
                            chainData[SvChain.CM_INT_TI], chainData[SvChain.CM_EXT_TI], chainData[SvChain.CM_INT_SHORT_TI], chainData[SvChain.CM_EXT_SHORT_TI],
                            chainData[SvChain.CM_INT_TI_CN_GAIN], chainData[SvChain.CM_EXT_TI_CN_GAIN], chainData[SvChain.CM_OVERLAPPING_TI],
                            chainData[SvChain.CM_DB], chainData[SvChain.CM_SHORT_DB], chainData[SvChain.CM_CHAIN_ENDS_FACE], chainData[SvChain.CM_CHAIN_ENDS_AWAY]));
                    ...
                    final int[] armClusterData = SvArmCluster.getArmClusterData(cluster);
                    long armClusterTIs = cluster.getArmClusters().stream().mapToInt(x -> x.getTICount()).sum();

                    mClusterFileWriter.write(String.format(",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
                            cluster.getArmClusters()
                                    .size(), armClusterTIs, armClusterData[SvArmCluster.ARM_CL_ISOLATED_BE], armClusterData[SvArmCluster.ARM_CL_TI_ONLY],
                            armClusterData[SvArmCluster.ARM_CL_DSB], armClusterData[SvArmCluster.ARM_CL_SIMPLE_DUP], armClusterData[SvArmCluster.ARM_CL_FOLDBACK],
                            armClusterData[SvArmCluster.ARM_CL_FOLDBACK_DSB], armClusterData[SvArmCluster.ARM_CL_COMPLEX_FOLDBACK], armClusterData[SvArmCluster.ARM_CL_COMPLEX_LINE],
                            armClusterData[SvArmCluster.ARM_CL_COMPLEX_OTHER]));
    ...
    private List<LinxViralInsertFile> generateViralInserts()
    {
        ...
        for(final SvVarData var : mAllVariants)
        {
            ...
            {
                viralInserts.add(new LinxViralInsertFile(mSampleId, var.dbId(), viralInsertData[ViralInsertAnnotator.VH_ID], viralInsertData[ViralInsertAnnotator.VH_NAME]));
            }
        }
