        ...
        for (Map.Entry<String, List<SvBreakend>> entry : chrBreakendMap.entrySet())
        {
            ...
            for (int i = 0; i < breakendList.size(); ++i)
            {
                ...
                if (var.type() == BND)
                {
                    ...
                    if (dbPair != null && dbPair.length() <= SvaConstants.MIN_DEL_LENGTH && dbPair.getOtherSV(var).type() == SGL
                ...
                else
                {
                    ...
                    for (int j = i + 1; j < breakendList.size(); ++j)
                    {
                        ...
                        if (nextBreakend.getSV().type() == BND)
                        {
                            ...
                            if (dbPair == null || dbPair != nextDbPair || dbPair.length() > SvaConstants.MIN_DEL_LENGTH)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(remoteDbPair != null && remoteDbPair.length() <= SvaConstants.MIN_DEL_LENGTH
                ...
                for (int j = i + 1; j < breakendList.size(); ++j)
                {
                    ...
                    if (Math.abs(nextBreakend.position() - breakend.position()) > proximityLength)
                        break;
                ...
                for (int j = i - 1; j >= 0; --j)
                {
                    ...
                    if (Math.abs(breakend.position() - prevBreakend.position()) > proximityLength)
                        break;
