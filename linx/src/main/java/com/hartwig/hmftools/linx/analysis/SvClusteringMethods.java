...
public class SvClusteringMethods {
    ...
    public void clusterByProximity(List<SvCluster> clusters)
    {
        ...
        for (final Map.Entry<String, List<SvBreakend>> entry : mChrBreakendMap.entrySet())
        {
            ...
            while (currentIndex < breakendList.size())
            {
                ...
                else if (Math.abs(nextBreakend.position() - breakend.position()) > mProximityDistance)
                {
    ...
    public static void checkClusteringClonalDiscrepancy(final SvVarData var1, final SvVarData var2, final String clusterReason)
    {
        ...
        if(var1.ploidyMin() > SvaConstants.LOW_CN_CHANGE_SUPPORT && var2.ploidyMin() > SvaConstants.LOW_CN_CHANGE_SUPPORT)
            return;
        ...
        for(int be1 = SvVarData.SE_START; be1 <= SvVarData.SE_END; ++be1)
        {
            ...
            {
                if (haveLinkedAssemblies(var1, var2, SvVarData.isStart(be1), SvVarData.isStart(be2)))
                {
    ...
    public static boolean hasLowCNChangeSupport(final SvVarData var)
    {
        ...
        if(var.isNullBreakend())
            return var.copyNumberChange(true) < SvaConstants.LOW_CN_CHANGE_SUPPORT;
        else
            return var.copyNumberChange(true) < SvaConstants.LOW_CN_CHANGE_SUPPORT && var.copyNumberChange(false) < SvaConstants.LOW_CN_CHANGE_SUPPORT;
    }
    ...
    private void mergeOnLOHEvents(final String sampleId, List<SvCluster> clusters)
    {
        ...
        for(final SvLOH lohEvent : lohList)
        {
            if(lohEvent.StartSV == SvLOH.LOH_NO_SV && lohEvent.EndSV == SvLOH.LOH_NO_SV)
                continue;
    ...
    private boolean mergeOnUnresolvedSingles(List<SvCluster> clusters)
    {
        ...
        for (final Map.Entry<String, List<SvBreakend>> entry : mChrBreakendMap.entrySet())
        {
            ...
            for(int i = 0; i < breakendList.size(); ++i)
            {
                ...
                long prevProximity = prevBreakend != null ? Math.abs(breakend.position() - prevBreakend.position()) : -1;
                long nextProximity = nextBreakend != null ? Math.abs(breakend.position() - nextBreakend.position()) : -1;
                ...
                if(nextBreakend != null && i < breakendCount - 2)
                {
                    ...
                    {
                        long followingProximity = Math.abs(nextBreakend.position() - followingBreakend.position());
                ...
                if(resolvedType != RESOLVED_TYPE_NONE)
                {
                    ...
                    otherCluster.setSyntheticData(Math.abs(otherVar.position(true) - var.position(true)), 0);
    ...
    private final String canResolveWithSoloSingle(SvCluster otherCluster, SvCluster soloSingleCluster)
    {
        ...
        if(otherCluster.getSvCount() == 1)
        {
            ...
            else
            {
                ...
                if(round(cnInconsistency) != Math.round(soloSingle.copyNumberChange(true)))
                    return RESOLVED_TYPE_NONE;
    ...
    public static String markSinglePairResolvedType(final SvVarData sgl1, final SvVarData sgl2)
    {
        ...
        long length = Math.abs(breakend1.position() - breakend2.position());
        ...
        else
        {
            ...
            if(length >= SvaConstants.MIN_DEL_LENGTH)
        ...
        for(Map.Entry<String, List<SvBreakend>> entry : mChrBreakendMap.entrySet())
        {
            ...
            for(int i = 0; i < breakendList.size(); ++i)
            {
                ...
                else if(var.type() == nextVar.type() && var.hasEquivBreakend() && nextVar.hasEquivBreakend())
                {
                    ...
                    && Math.abs(otherBe.position() - nextOtherBe.position()) <= PERMITED_DUP_BE_DISTANCE)
                    {
        ...
        for(Map.Entry<String, List<SvBreakend>> entry : mChrBreakendMap.entrySet())
        {
            ...
            for(int i = 0; i < breakendList.size(); ++i)
            {
                ...
                if((prevBreakend != null && prevBreakend.getSV() == var) || (nextBreakend != null && nextBreakend.getSV() == var))
                    relationType = SvVarData.RELATION_TYPE_NEIGHBOUR;
                else
                    relationType = SvVarData.RELATION_TYPE_OVERLAP;
