...
public class ClusterAnalyser {
    ...
    public boolean clusterAndAnalyse()
    {
        ...
        mClusters.forEach(x -> LineElementAnnotator.markLineCluster(x, mClusteringMethods.getProximityDistance()));
    ...
    public void applyCopyNumberReplication(SvCluster cluster)
    {
        ...
        SvCluster.isSpecificCluster(cluster);
        ...
        int replicationCap = mConfig.ChainingSvLimit > 0 ? Math.min(mConfig.ChainingSvLimit, SvaConstants.DEFAULT_CHAINING_SV_LIMIT) : SvaConstants.DEFAULT_CHAINING_SV_LIMIT;
    ...
    private boolean mergeLOHResolvingClusters(List<SvCluster> clusters)
    {
        ...
        for(SvCluster lohCluster : clustersWithLohEvents)
        {
            ...
            while(lohIndex < lohEvents.size())
            {
                ...
                {
                    SvBreakend lohBreakend = lohEvent.getBreakend(SvVarData.isStart(be));
            ...
            for (final Map.Entry<String, List<SvBreakend>> entry : cluster.getChrBreakendMap().entrySet())
            {
                ...
                for(int armIndex = 0; armIndex <= 1; ++armIndex)
                {
                    ...
                    for(int directionIndex = 0; directionIndex <= 1; ++directionIndex)
                    {
                        ...
                        double armEndMAP = facingCentromere ? centromereCNData[CnDataLoader.CN_SEG_DATA_MAP_BEFORE] : telomereMAP;
    ...
    private boolean canMergeClustersOnFoldbacks(final SvCluster cluster1, final SvCluster cluster2)
    {
        ...
        if (!cluster1Foldbacks.isEmpty() && !cluster2Foldbacks.isEmpty())
        {
            for (final SvVarData var1 : cluster1Foldbacks)
            {
                for (int be1 = SvVarData.SE_START; be1 <= SvVarData.SE_END; ++be1)
                {
                    ...
                    if (be1 == SvVarData.SE_END && var1.type() != BND)
                        continue;
                    ...
                    for (final SvVarData var2 : cluster2Foldbacks)
                    {
                        ...
                        {
                            boolean v2Start = SvVarData.isStart(be2);

                            if (be2 == SvVarData.SE_END && var2.type() != BND)
                                continue;
        ...
        for (int i = 0; i <= 1; ++i)
        {
            ...
            for (final SvVarData var : foldbacks)
            {
                ...
                if (Math.abs(nextBreakend.position() - foldbackBreakend.position()) > SvaConstants.MAX_FOLDBACK_NEXT_CLUSTER_DISTANCE)
                    continue;
    ...
    private boolean canMergeClustersOnCommonArms(final SvCluster cluster1, final SvCluster cluster2, long armWidthCutoff)
    {
        ...
        SvCluster.areSpecificClusters(cluster1, cluster2);
        ...
        for (final SvVarData var1 : crossArmList1)
        {
            ...
            {
                if(!SvVarData.haveSameChrArms(var1, var2))
                    continue;
    ...
    private boolean checkFoldbackBreakends(SvBreakend beStart, SvBreakend beEnd)
    {
        ...
        else
        {
            ...
            if(chainData[SvChain.CHAIN_LINK_COUNT] == 0 ) // || chainData[CHAIN_LINK_COUNT] != chainData[CHAIN_ASSEMBLY_LINK_COUNT]
                return false;

            int chainLength = chainData[SvChain.CHAIN_LENGTH];

            if(chainLength > SvaConstants.MAX_FOLDBACK_CHAIN_LENGTH)
            {
                /*
            ...
            chainInfo = String.format("%d;%d;%d",
                    chainData[SvChain.CHAIN_LINK_COUNT], chainData[SvChain.CHAIN_ASSEMBLY_LINK_COUNT], chainLength);
        }

        int length = (int) Math.abs(beEnd.position() - beStart.position());
    ...
    private void checkReplicatedBreakendFoldback(SvBreakend be)
    {
        ...
        if(!var.getAssemblyMatchType(true).equals(SvLinkedPair.ASSEMBLY_MATCH_MATCHED)
        && !var.getAssemblyMatchType(false).equals(SvLinkedPair.ASSEMBLY_MATCH_MATCHED))
        {
            return;
        }
