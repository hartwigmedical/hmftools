...
public class CnDataLoader
{
        ...
        for (final StructuralVariantData svData : mSvDataList)
        {
            ...
            for(int be = SvVarData.SE_START; be <= SvVarData.SE_END; ++be)
            {
                ...
                for(SvCNData cnData : cnDataList)
                {
                    ...
                    {
                        if(svData.type() == SGL && svData.filter().equals(SvVarData.NONE_SEGMENT_INFERRED) && cnData.matchesSegment(NONE, true))
                        {
                    ...
                    if(cnDataItems == null)
                    {
                        cnDataItems = new SvCNData[SvVarData.SE_END +1];
            ...
            for(int index = 0; index < cnDataList.size(); ++index)
            {
                ...
                if (isLohSection || lohOnStartTelomere)
                {
                    ...
                    if (lohRegained || reset)
                    {
                        ...
                        if (lohRegained && cnData.EndPos - cnData.StartPos <= SvaConstants.SHORT_TI_LENGTH && index < cnDataList.size() - 1)
                        {
            ...
            double lastMinCN, double lohMinCN, int segCount, boolean incomplete, boolean skipped, boolean isValid)
    {
        ...
        SvLOH lohData = new SvLOH(
                ...
                startSvData != null ? startSvData.id() : SvLOH.LOH_NO_SV,
                endSvData != null ? endSvData.id() : SvLOH.LOH_NO_SV,
                skipped, isValid);
    ...
    private void reaclcAdjustedPloidy(final String sampleId)
    {
        ...
        for (Map.Entry<Integer, SvCNData[]> entry : mSvIdCnDataMap.entrySet())
        {
            ...
            final SvCNData cnStartData = cnDataPair[SvVarData.SE_START];
            ...
            final SvCNData cnEndNextData = cnDataPair[SvVarData.SE_END];
