...
import static com.hartwig.hmftools.linx.fusion.SvDisruptionAnalyser.DRUP_TSG_GENES_FILE;
import static com.hartwig.hmftools.linx.fusion.SvDisruptionAnalyser.markNonDisruptiveTranscripts;
import static com.hartwig.hmftools.linx.fusion.SvFusionAnalyser.couldBeReportable;
import static com.hartwig.hmftools.linx.fusion.SvFusionAnalyser.determineReportableFusion;
import static com.hartwig.hmftools.linx.fusion.SvFusionAnalyser.validFusionTranscript;
...
public class FusionDisruptionAnalyser
{
    ...
    public void initialise(final CommandLine cmdLineArgs, final String outputDir, final SvaConfig config, SvGeneTranscriptCollection ensemblDataCache)
    {
        ...
        if(cmdLineArgs != null)
        {
            ...
            if (cmdLineArgs.hasOption(PRE_GENE_BREAKEND_DISTANCE))
            {
                ...
                SvGeneTranscriptCollection.PRE_GENE_PROMOTOR_DISTANCE = preGeneBreakendDistance;
            }
    ...
    {
        int upstreamDistance = applyPromotorDistance ? SvGeneTranscriptCollection.PRE_GENE_PROMOTOR_DISTANCE : 0;
        ...
        if(selectiveLoading)
        {
            ...
            for (final SvVarData var : svList)
            {
                ...
                {
                    if (be == SvVarData.SE_END && var.isNullBreakend())
                        continue;
    ...
    private void uploadData(final List<SvVarData> svList, final DatabaseAccess dbAccess)
    {
        ...
        for (SvVarData var : svList)
        {
            for (int be = SvVarData.SE_START; be <= SvVarData.SE_END; ++be)
            {
                for (GeneAnnotation geneAnnotation : var.getGenesList(SvVarData.isStart(be)))
                {
    ...
    private FusionTermination checkTranscriptDisruptionInfo(final SvBreakend breakend, final Transcript transcript)
    {
        ...
        while(true)
        {
            ...
            if (nextBreakend.orientation() != breakend.orientation())
            {
                ...
                long breakendDistance = Math.abs(breakend.position() - nextBreakend.position());
    ...
    private void findDisruptions(final List<SvVarData> svList)
    {
        ...
        for (final SvVarData var : svList)
        {
            for (int be = SvVarData.SE_START; be <= SvVarData.SE_END; ++be)
            {
                ...
                final List<GeneAnnotation> tsgGenesList = var.getGenesList(SvVarData.isStart(be)).stream()
