...
public class DriverGeneAnnotator
{
    ...
    private void writeDriverData(final DriverGeneData driverGeneData)
    {
        try
        {
            ...
            int dataCount = Math.max(1, Math.max(svBreakends.size(), driverGeneData.getSvInfoList().size()));
            ...
            for(int i = 0; i < dataCount; ++i)
            {
                ...
                writer.write(String.format(",%.2f,%s,%s,%.2f,%.2f,%.2f",
                        ...
                        cnData[CnDataLoader.CENTROMERE_CN],
                        arm == CHROMOSOME_ARM_P ? cnData[CnDataLoader.P_ARM_TELOMERE_CN] : cnData[CnDataLoader.Q_ARM_TELOMERE_CN]));
