...
public class LinkFinder
{
        ...
        for (final Map.Entry<String, List<SvBreakend>> entry : chrBreakendMap.entrySet())
        {
            ...
            for (int i = 0; i < breakendList.size() -1; ++i)
            {
                ...
                for (int j = i+1; j < breakendList.size(); ++j)
                {
                    ...
                    SvLinkedPair newPair = new SvLinkedPair(lowerSV, upperSV, SvLinkedPair.LINK_TYPE_TI, v1Start, v2Start);
                    newPair.setIsAssembled();
                    lowerSV.setAssemblyMatchType(SvLinkedPair.ASSEMBLY_MATCH_MATCHED, v1Start);
                    upperSV.setAssemblyMatchType(SvLinkedPair.ASSEMBLY_MATCH_MATCHED, v2Start);
            ...
            for(int be1 = SvVarData.SE_START; be1 <= SvVarData.SE_END; ++be1)
            {
                ...
                for (int j = i+1; j < svList.size(); ++j)
                {
                    ...
                    for(int be2 = SvVarData.SE_START; be2 <= SvVarData.SE_END; ++be2)
                    {
                        ...
                        long distance = Math.abs(var1.position(v1Start) - var2.position(v2Start));
                        int minTiLength = Math.max(var1.getMinTemplatedLength(v1Start), var2.getMinTemplatedLength(v2Start));
                        ...
                        if(newPair.linkType() == SvLinkedPair.LINK_TYPE_DB)
                        {
    ...
    public static int getMinTemplatedInsertionLength(SvBreakend breakend1, SvBreakend breakend2)
    {
        ...
        return Math.max(breakend1.getMinTemplatedLength(), breakend2.getMinTemplatedLength());
    }
        ...
        for (final Map.Entry<String, List<SvBreakend>> entry : chrBreakendMap.entrySet())
        {
            ...
            for (int i = 0; i < breakendList.size() - 1; ++i)
            {
                ...
                if(breakend.orientation() == 1 && nextBreakend.orientation() == -1)
                {
                    ...
                    SvLinkedPair dbPair = new SvLinkedPair(var1, var2, SvLinkedPair.LINK_TYPE_DB, breakend.usesStart(), nextBreakend.usesStart());
                ...
                else if(distance < minTiLength)
                {
                    ...
                    SvLinkedPair dbPair = new SvLinkedPair(var1, var2, SvLinkedPair.LINK_TYPE_TI, breakend.usesStart(), nextBreakend.usesStart());
                    markDeletionBridge(dbPair);
                }
            }
        }
