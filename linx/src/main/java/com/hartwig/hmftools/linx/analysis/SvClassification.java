...
public class SvClassification
{
    ...
    public static void markSyntheticTypes(SvCluster cluster, long delDupLongThreshold, int proximityThreshold)
    {
        ...
        else if (cluster.getSvCount() == 2 && cluster.isConsistent() && cluster.getTypeCount(SGL) == 2)
        {
            ...
            if(resolvedType != RESOLVED_TYPE_NONE)
            {
                ...
                long length = Math.abs(sgl1.position(true) - sgl2.position(true));
    ...
    public static void markSyntheticDelDups(SvCluster cluster, long delDupLongThreshold)
    {
        ...
        long syntheticLength = Math.abs(startBreakend.position() - endBreakend.position());
        ...
        else if(longTICount == 1)
        {
            ...
            if(!faceAway && syntheticLength > SvaConstants.SHORT_TI_LENGTH)
            {
    ...
    private static void classifyReciprocalDupPairs(SvCluster cluster, SvChain chain, SvLinkedPair longestPair)
    {
        ...
        if(!longestPair.chromosome().equals(startBreakend.chromosome()))
        {
            ...
            for (SvLinkedPair pair : chain.getLinkedPairs())
            {
                ...
                if (pair.length() <= SvaConstants.SHORT_TI_LENGTH)
                {
                    ...
                    longestTILength = Math.max(pair.length(), longestTILength);
                }
                else
                {
        ...
        else
        {
            ...
            List<SvLinkedPair> linksToAdd = chain.getLinkedPairs().stream()
                    .filter(x -> x.length() <= SvaConstants.SHORT_TI_LENGTH)
            ...
            syntheticLength = Math.abs(newChain.getOpenBreakend(true).position() - newChain.getOpenBreakend(false).position());
    ...
    public static void markSyntheticReciprocalInversion(SvCluster cluster, int proximityThreshold)
    {
        ...
        for (SvLinkedPair pair : chain.getLinkedPairs())
        {
            ...
            {
                if(tiPair.length() > SvaConstants.SHORT_TI_LENGTH)
        ...
        {
            syntheticLength = Math.abs(startBe2.position() - endBe2.position());
        }
        else
        {
            return;
