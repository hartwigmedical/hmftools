...
import static com.hartwig.hmftools.bamtools.fastqbiomodalcollapse.NeedlemanWunschAligner.SuffixPenaltyOption.PENALIZE;
...
//                }
    ...
    private static void stripNulls(final List<BaseQualPair> seq)
    {
        while(!seq.isEmpty() && seq.get(seq.size() - 1) == null)
        {
    ...
    private void processFastqPair(final BufferedWriter writer, final FastqRecord fastq1, final FastqRecord fastq2) throws IOException
    {
        ...
        List<Pair<BaseQualPair, BaseQualPair>> fullFragmentSeq = ALIGNER.align(seq1, seqRevComp2, FastqBiomodalCollapse::getExactScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, true, NO_PENALTY_SINGLE);

        if(fullFragmentSeq.stream().mapToInt(x -> x.getLeft() == null || x.getRight() == null ? 0 : 1).sum() == 0)
        {
            fullFragmentSeq = Lists.newArrayList();
            for(int i = 0; i < seq1.size(); i++)
            {
                fullFragmentSeq.add(Pair.of(seq1.get(i), null));
            }

            for(int i = 0; i < seqRevComp2.size(); i++)
            {
                fullFragmentSeq.add(Pair.of(null, seqRevComp2.get(i)));
            }
        }
        ...
        if(fullFragmentHairpin != null)
        {
            ...
            alignedSeq = ALIGNER.align(read1, read2, FastqBiomodalCollapse::getModCScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, false, PENALIZE);
            consensusSeq = collapseAlignedSeq(alignedSeq);
            cigar = getCigar(alignedSeq);

            alignedSeqRevComp = ALIGNER.align(read2_b, read1_b, FastqBiomodalCollapse::getModCScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, true, PENALIZE);
            consensusSeqRevComp = collapseAlignedSeq(alignedSeqRevComp);
            cigarRevComp = getCigar(alignedSeqRevComp);

            finalAlignedSeq = ALIGNER.align(consensusSeq, consensusSeqRevComp, FastqBiomodalCollapse::getExactScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, true, PENALIZE);
        ...
        else
        {
            ...
            alignedSeq = ALIGNER.align(read1, read2, FastqBiomodalCollapse::getModCScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, false, NO_PENALTY_SINGLE);
            consensusSeq = collapseAlignedSeq(alignedSeq);
            cigar = getCigar(alignedSeq);
            ...
            finalAlignedSeq = ALIGNER.align(consensusSeq, consensusSeqRevComp, FastqBiomodalCollapse::getExactScore, OPEN_GAP_PENALTY, EXTEND_GAP_PENALTY, true, NO_PENALTY_SINGLE);
            finalConsensusSeq = collapseAlignedSeqExact(finalAlignedSeq);
            finalCigar = getCigar(finalAlignedSeq);
        }
        ...
        statLine.add(String.valueOf(fullFragmentHairpin.PrefixMatchLength));

        statLine.add(seqReadString(read1));
        statLine.add(seqBaseQualString(read1));
        statLine.add(seqReadString(read2));
        statLine.add(seqBaseQualString(read2));
        statLine.add(seqReadString(read2_b));
        statLine.add(seqBaseQualString(read2_b));
        statLine.add(seqReadString(read1_b));
        statLine.add(seqBaseQualString(read1_b));
        statLine.add(cigar);
        statLine.add(consensusReadForOutput(seqReadString(consensusSeq)));
        statLine.add(seqBaseQualString(consensusSeq));
        statLine.add(cigarRevComp);
        statLine.add(consensusReadForOutput(seqReadString(consensusSeqRevComp)));
        statLine.add(seqBaseQualString(consensusSeqRevComp));
    ...
    private static final String[] STAT_HEADERS = {
            ...
            "full_fragment_consensus_hairpin_suffix_match_length",
            "read1",
            "qual1",
            "read2",
            "qual2",
