...
public class LowPurityTransformer
{
    ...
    private static FindingRecord updateQCLowPurityStatus(@NotNull FindingRecord record)
    {
        boolean hasLowPurity = hasLowPurity(record.somaticSmallVariants().status())
                ...
                || hasLowPurity(record.rnaStatistics().status())
                || hasLowPurity(record.highExpressionGenes().status())
                || hasLowPurity(record.lowExpressionGenes().status())
                || hasLowPurity(record.rnaFusions().status())
                || hasLowPurity(record.novelSpliceJunctions().status())
