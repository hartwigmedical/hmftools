...
public class JunctionRacFragments
{
    ...
    {
        return orientation == ORIENT_FWD ? mPosJunctionFragments : mNegJunctionFragments;
    }
    ...
    {
        Map<Integer,List<FusionFragment>> junctionGroups = orientation == ORIENT_FWD ? mPosJunctionFragments : mNegJunctionFragments;
    ...
    public boolean checkAddCandidateGroup(final ChimericReadGroup readGroup)
    {
        ...
        for(int i = 0; i <= 1; ++i)
        {
            byte juncOrientation = (i == 0) ? ORIENT_FWD : ORIENT_REV;
