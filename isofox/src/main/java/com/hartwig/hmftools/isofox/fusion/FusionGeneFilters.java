...
public class FusionGeneFilters
{
    ...
    private final List<SvRegion> mRestrictedGeneRegions;
    private final List<SvRegion> mExcludedGeneRegions;
    ...
    private void buildGeneRegions(final EnsemblDataCache geneTransCache)
    {
        mConfig.EnrichedGeneIds.stream()
                ...
                .filter(x -> x != null)
                .forEach(x -> mExcludedGeneRegions.add(new SvRegion(
        ...
        mConfig.RestrictedGeneIds.stream()
                .map(x -> geneTransCache.getGeneDataById(x))
                .filter(x -> x != null)
                .forEach(x -> mRestrictedGeneRegions.add(new SvRegion(
