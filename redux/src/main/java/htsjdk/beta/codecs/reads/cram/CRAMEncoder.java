...
public abstract class CRAMEncoder implements ReadsEncoder {
    ...
    @Override
    public void setHeader(final SAMFileHeader samFileHeader, boolean writeHeader) {
        ...
        cramFileWriter = getCRAMWriter(samFileHeader, readsEncoderOptions, writeHeader);
    }
    ...
    private CRAMFileWriter getCRAMWriter(final SAMFileHeader samFileHeader, final ReadsEncoderOptions readsEncoderOptions, boolean writeHeader) {
        ...
        if (readsEncoderOptions.isPreSorted()) {
            ...
            if (outputResource.getIOPath().isPresent()) {
                cramFileWriter = new CRAMFileWriter(
                        ...
                        samFileHeader,
                        outputResource.getIOPath().get().toString(), writeHeader);
                ...
                cramFileWriter = new CRAMFileWriter(
                        ...
                        getCRAMReferenceSource(readsEncoderOptions.getCRAMEncoderOptions()),
                        samFileHeader,
                        outputResource.getDisplayName(), writeHeader);
                return cramFileWriter;
            }
        } else {
            ...
            if (readsResource.getIOPath().isPresent()) {
                return samFileWriterFactory.makeCRAMWriter(
                        ...
                        cramEncoderOptions.getReferencePath().get().toPath(), writeHeader);
            } else {
