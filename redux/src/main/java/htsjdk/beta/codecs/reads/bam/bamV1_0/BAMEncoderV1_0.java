...
public class BAMEncoderV1_0 extends BAMEncoder {
    ...
    public void setHeader(final SAMFileHeader samFileHeader, boolean writeHeader) {
        ...
        samFileWriter = getBAMFileWriter(getReadsEncoderOptions(), samFileHeader, writeHeader);
    }
    ...
    private SAMFileWriter getBAMFileWriter(
            ...
            final SAMFileHeader samFileHeader, boolean writeHeader) {
        ...
        if (readsResource.getIOPath().isPresent()) {
            return samFileWriterFactory.makeBAMWriter(
                    samFileHeader,
                    preSorted,
                    readsResource.getIOPath().get().toPath(), writeHeader);
        } else {
            return samFileWriterFactory.makeBAMWriter(
                    samFileHeader,
                    preSorted,
                    readsResource.getOutputStream().get(), writeHeader);
        }
    }
