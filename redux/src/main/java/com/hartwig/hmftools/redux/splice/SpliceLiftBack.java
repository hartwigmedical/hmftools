...
public class SpliceLiftBack
{
    ...
    private LiftBackResources buildResources(final List<ContigEntry> contigEntries)
    {
        ...
        if(mConfig.RescueViaSupp || mConfig.ExtendSoftclipTails)
        {
            try
            {
                ...
                RD_LOGGER.info("loaded {} annotated junctions from {}", annotated.size(), mConfig.EnsemblDataDir);
            }
            catch(IOException e)
            {
        ...
        if(mConfig.RnaUnmapRegionsFile != null)
        {
            ...
            RD_LOGGER.info("loaded pre-liftback excluded regions from {}", mConfig.RnaUnmapRegionsFile);
        }
    ...
    private void mergeWorkerStats(final List<LiftBackWorker> workers)
    {
        ...
        {
            RD_LOGGER.warn("failed to write liftback summary: {}", e.toString());
        }
        ...
        if(excludedReads > 0)
            RD_LOGGER.info("pre-liftback excluded-region reads dropped: {}", excludedReads);
    }
