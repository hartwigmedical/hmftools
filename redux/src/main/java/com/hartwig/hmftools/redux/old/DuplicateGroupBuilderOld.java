...
public class DuplicateGroupBuilderOld
{
    private final UmiConfig mUmiConfig;
    private final Statistics mStats;
    private final boolean mFormConsensus;
    private final UmiGroupBuilderOld mUmiGroupBuilder;

    public DuplicateGroupBuilderOld(final ReduxConfig config)
    {
        mFormConsensus = config.FormConsensus;
        mUmiConfig = config.UMIs;
        mStats = new Statistics();
        mUmiGroupBuilder = new UmiGroupBuilderOld(config.UMIs, mStats.UmiStats);
    }
    ...
    public static void findDuplicateFragments(
            final List<FragmentOld> fragments, final List<FragmentOld> resolvedFragments,
            final List<List<FragmentOld>> positionDuplicateGroups, final List<CandidateDuplicates> candidateDuplicatesList,
            boolean requireOrientationMatch)
    {
        ...
        if(fragments.size() == 1)
        {
            FragmentOld fragment = fragments.get(0);
        ...
        Map<FragmentOld,List<FragmentOld>> possibleDuplicates = Maps.newHashMap();
        Map<FragmentOld, FragmentOld> linkedDuplicates = Maps.newHashMap();

        int i = 0;
        while(i < fragments.size())
        {
            FragmentOld fragment1 = fragments.get(i);
            ...
            FragmentOld existingLinkedFragment1 = linkedDuplicates.get(fragment1);

            boolean isCandidateDup = existingLinkedFragment1 != null;

            List<FragmentOld> candidateFragments = existingLinkedFragment1 != null ? possibleDuplicates.get(existingLinkedFragment1) : null;

            int j = i + 1;
            while(j < fragments.size())
            {
                FragmentOld fragment2 = fragments.get(j);
                ...
                if(fragment1.status() != DUPLICATE && status == CANDIDATE)
                {
                    ...
                    FragmentOld existingLinkedFragment2 = linkedDuplicates.get(fragment2);
                    ...
                    {
                        List<FragmentOld> existingGroup = possibleDuplicates.get(existingLinkedFragment2);
                        ...
                        else
                        {
                            ...
                            for(FragmentOld fragment : candidateFragments)
                            {
        ...
        {
            for(List<FragmentOld> candidateFragments : possibleDuplicates.values())
            {
                List<FragmentOld> unresolvedFragments = candidateFragments.stream().filter(x -> !x.status().isResolved()).collect(Collectors.toList());
                ...
                {
                    FragmentOld fragment = unresolvedFragments.get(0);
                    fragment.setStatus(NONE);
                    resolvedFragments.add(fragment);
                }
            }
        ...
        if(candidateCount + resolvedFragments.size() != fragmentCount)
        {
            ...
            for(FragmentOld fragment : resolvedFragments)
            {
            ...
            for(CandidateDuplicates candidateDuplicates : candidateDuplicatesList)
            {
                for(FragmentOld fragment : candidateDuplicates.fragments())
                {
    ...
    public List<DuplicateGroupOld> processDuplicateGroups(
            final List<List<FragmentOld>> rawDuplicateGroups, boolean captureStats, final List<FragmentOld> singleFragments)
    {
        ...
        {
            List<DuplicateGroupOld> umiGroups = mUmiGroupBuilder.processUmiGroups(rawDuplicateGroups, singleFragments, captureStats);

            if(captureStats)
            {
                for(DuplicateGroupOld umiGroup: umiGroups)
                {
        ...
        if(captureStats)
        {
            for(List<FragmentOld> fragments : rawDuplicateGroups)
            {
    ...
    private List<DuplicateGroupOld> processConsensusGroups(final List<List<FragmentOld>> rawDuplicateGroups)
    {
        List<DuplicateGroupOld> duplicateGroups = Lists.newArrayListWithCapacity(rawDuplicateGroups.size());

        for(List<FragmentOld> fragments : rawDuplicateGroups)
        {
            DuplicateGroupOld duplicateGroup = new DuplicateGroupOld(null, fragments.get(0));
    ...
    private void processNonUmiGroups(final List<List<FragmentOld>> duplicateGroups)
    {
    ...
    private static void setPrimaryRead(final List<FragmentOld> duplicateFragments)
    {
        FragmentOld primary = findPrimaryFragment(duplicateFragments, false);
        primary.setStatus(PRIMARY);
    }

    public static FragmentOld findPrimaryFragment(final List<FragmentOld> fragments, boolean considerMarkedDups)
    {
        if(considerMarkedDups)
        {
            // take the primary (non-duplicate) group if there is (just) one already marked
            List<FragmentOld> nonDupGroups = fragments.stream().filter(x -> !hasDuplicates(x)).collect(Collectors.toList());
    ...
    private static boolean hasDuplicates(final FragmentOld fragment)
    {
        return fragment.reads().stream().anyMatch(x -> x.getDuplicateReadFlag());
    }

    public static double calcBaseQualAverage(final FragmentOld fragment)
    {
        int readBaseCount = 0;
